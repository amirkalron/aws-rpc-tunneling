...
public class RestTunnelSender extends RestTunnelClient {
	...
	public <T> T post(String url, Object body, Class<T> returnType)
			throws Exception {
	...
	private <T> T sendRequest(String method, Object body, Class<T> returnType)
			throws Exception {
		// wrap the message in JSON format and send it
		String jsonBody = (body == null || "".equals(body)) ? "" : json
				.toJson(body);
		RestTunnelMessage request = new RestTunnelMessage(method, jsonBody,
		...
		T retval = new JsonResponseExtractor<T>().extractData(
				response.getBody(), returnType);
		return retval;
	};
	...
	public void listenToResponses() throws Exception {
		...
		listen(queue_responses, new QueueListener() {
			public void onMessageReceived(String jsonMsg) {
				try {
					RestTunnelMessage response = json.fromJson(jsonMsg,
							RestTunnelMessage.class);
					RestTunnelMessage request = requests.get(response
							.getReqUUid());
					if (request != null) {
						...
						System.out.println("No request found for "
								+ response.getReqUUid());
				} catch (Exception exp) {
					exp.printStackTrace();
				}
